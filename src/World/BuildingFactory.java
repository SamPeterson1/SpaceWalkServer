...
public class BuildingFactory {
	...
	public BuildingFactory(String packageName) {
		try {
			String[] classNames;
	...
	public Building getBuilding(int id) {
		try {
			return (Building) Class.forName(this.registations.get(Integer.valueOf(id))).getConstructor().newInstance();
		} catch (Exception e) {
			e.printStackTrace();
			return null;
		}
	...
	private void registerClasses(String[] classNames) {
		try {
			for (String className : classNames) {
				Building tile = (Building) Class.forName(className).getConstructor().newInstance();
